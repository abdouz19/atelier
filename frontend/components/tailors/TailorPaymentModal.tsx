    ...
    <AppModal
      ...
      footer={
        <>
          ...
          <button type="submit" form="tailor-payment-form" disabled={isSubmitting} className="rounded-lg bg-primary-500 px-4 py-2 text-sm font-medium text-white hover:bg-primary-600 disabled:opacity-60">
    ...
    >
      <form id="tailor-payment-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <FormField label="المبلغ" error={errors.amount?.message} required>
          <input type="number" step="any" {...register('amount')} className="w-full rounded-lg border border-border px-3 py-2 text-sm outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20" />
        </FormField>
        <FormField label="التاريخ" error={errors.paymentDate?.message} required>
          <input type="date" {...register('paymentDate')} className="w-full rounded-lg border border-border px-3 py-2 text-sm outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20" />
        </FormField>
