...
export function TailorDetail({ id, onBack }: TailorDetailProps) {
  ...
  if (loading) return <div className="p-6 text-text-muted text-center">جاري التحميل...</div>;
    ...
    <div dir="rtl" className="space-y-6">
      ...
      <div className="rounded-xl border border-border bg-surface overflow-hidden">
        ...
        {detail.sewingTransactions.length === 0 ? (
          ...
          <table className="w-full text-sm"><thead className="bg-base/60 text-xs text-text-muted"><tr>
            ...
            {detail.sewingTransactions.map(t => (
              <tr key={t.batchId} className="hover:bg-base/60">
      ...
      <div className="rounded-xl border border-border bg-surface overflow-hidden">
        ...
        {detail.payments.length === 0 ? (
          <p className="px-4 py-6 text-center text-sm text-text-muted">لا توجد دفعات بعد</p>
        ) : (
          <div className="divide-y divide-border">{detail.payments.map(p => (
