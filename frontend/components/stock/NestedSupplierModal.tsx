      ...
      <div className="w-full max-w-md rounded-xl bg-surface p-6 shadow-xl">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
          <div>
            ...
            <input {...register('name')} className="w-full rounded-lg border border-border px-3 py-2 text-sm text-text-base input-transition outline-none focus:border-primary-500" />
            {errors.name && <p className="mt-1 text-xs text-red-600">{errors.name.message}</p>}
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-text-base">رقم الهاتف (اختياري)</label>
            <input {...register('phone')} className="w-full rounded-lg border border-border px-3 py-2 text-sm text-text-base input-transition outline-none focus:border-primary-500" />
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-text-base">العنوان (اختياري)</label>
            <input {...register('address')} className="w-full rounded-lg border border-border px-3 py-2 text-sm text-text-base input-transition outline-none focus:border-primary-500" />
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-text-base">المنتجات (اختياري)</label>
            <input {...register('productsSold')} className="w-full rounded-lg border border-border px-3 py-2 text-sm text-text-base input-transition outline-none focus:border-primary-500" />
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-text-base">ملاحظات (اختيارية)</label>
            <textarea {...register('notes')} rows={2} className="w-full rounded-lg border border-border px-3 py-2 text-sm text-text-base input-transition outline-none focus:border-primary-500" />
          </div>
