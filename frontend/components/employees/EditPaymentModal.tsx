      ...
      <div className="w-full max-w-sm rounded-xl bg-surface p-6 shadow-xl">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            <input type="number" step="any" {...register('amount')} className="w-full rounded-lg border border-border px-3 py-2 text-sm input-transition focus:border-primary-500 focus:outline-none" />
            {errors.amount && <p className="mt-1 text-xs text-red-500">{errors.amount.message}</p>}
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium">التاريخ *</label>
            <input type="date" {...register('paymentDate')} className="w-full rounded-lg border border-border px-3 py-2 text-sm input-transition focus:border-primary-500 focus:outline-none" />
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={onClose} className="rounded-lg border border-border px-4 py-2 text-sm text-text-base hover:bg-base/60">إلغاء</button>
            <button type="submit" disabled={isSubmitting} className="btn-tactile rounded-lg bg-primary-500 px-4 py-2 text-sm font-medium text-white hover:bg-primary-600 disabled:opacity-60">
